...
def instance_generator_NETGEN(nb_origins, nb_destinations, nb_transhipment_origins, nb_transhipment_destintions, nb_transhipment,
                                nb_arcs, capacity_upper_bound, capacity_lower_bound, total_demand):
...
if __name__ == "__main__":
    # temp = time.time()
    from mcnf_heuristics import is_strongly_connected
    for i in range(1000):
        print(i)
        size = 150
        nb_timesteps = 100
        graph, commodity_list, initial_path_list, origin_list = generate_instance("random_connected", (size, 5/size, int(size * 0.1), 1), {"max_demand" : 1})
        ...
        for timestep in range(nb_timesteps + 1):
            ...
            assert is_strongly_connected(graph_copy)
