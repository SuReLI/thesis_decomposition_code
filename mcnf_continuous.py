...
def run_arc_path_model(graph, commodity_list, model, path_and_var_per_commodity, convexity_constraint_dict, capacity_constraint_dict, overload_var, path_and_var, forbidden_arcs_per_commodity=None, commodity_indices_to_check=None, nb_iterations=None, flow_penalisation=0, var_delete_proba=0):
    nb_commodities = len(commodity_list)
    nb_nodes = len(graph)
    demand_list = [commodity[2] for commodity in commodity_list]
    arc_list = [(node, neighbor) for node in range(nb_nodes) for neighbor in graph[node]]
    ...
    if commodity_indices_to_check is None:
        commodity_indices_to_check = range(nb_commodities)
    ...
    for iter_index in range(nb_iterations):
        ...
        if obj_val > old_obj_val - 10**-3:
            var_delete_proba = 0


        if True or model.Params.OutputFlag:
            print("Nb variables = ", sum(len(l) for l in path_and_var_per_commodity))
            print(model.ObjVal)

        var_to_remove = []
        for commodity_index, l in enumerate(path_and_var_per_commodity):
            for index, var_tuple in enumerate(l):
                path, var = var_tuple
                if var.Vbasis != 0 and random.random() < var_delete_proba and var.Obj == 0:
                    var_to_remove.append((commodity_index, index))

        var_to_remove.reverse()

        for commodity_index, index in var_to_remove:
            path, var = path_and_var_per_commodity[commodity_index][index]
            remove_var_from_model(model, path_and_var_per_commodity, path_and_var, path, var, commodity_index)


        nb_new_vars = 0

        convexity_dual_var_list = [convexity_constraint_dict[commodity_index].Pi for commodity_index in range(nb_commodities)]

        for commodity_index in commodity_indices_to_check:
            origin, destination, demand = commodity_list[commodity_index]
            capacity_dual_var_graph = [{neighbor : -sum(constraint.Pi * coeff_list[commodity_index] for constraint, coeff_list in capacity_constraint_dict[node, neighbor]) + flow_penalisation for neighbor in graph[node]} for node in range(nb_nodes)]

            if forbidden_arcs_per_commodity is not None:
                for node, neighbor in forbidden_arcs_per_commodity[commodity_index]:
                    capacity_dual_var_graph[node].pop(neighbor)

            shortest_path, path_cost = dijkstra(capacity_dual_var_graph, origin, destination)
            reduced_cost = path_cost - convexity_dual_var_list[commodity_index]

            if reduced_cost < -10**-3:
                nb_new_vars += 1
                ...
                for node_index in range(len(shortest_path)-1):
                    node, neighbor = shortest_path[node_index], shortest_path[node_index+1]

                    for constraint, coeff_list in capacity_constraint_dict[node, neighbor]:
                        column.addTerms(coeff_list[commodity_index], constraint)

                new_var = model.addVar(obj=flow_penalisation * (len(shortest_path)-1) * demand, column=column)
                path_and_var_per_commodity[commodity_index].append((shortest_path, new_var))

                for node_index in range(len(shortest_path)-1):
                    node, neighbor = shortest_path[node_index], shortest_path[node_index+1]

                    if commodity_index not in path_and_var[node, neighbor]:
                        path_and_var[node, neighbor][commodity_index] = []

                    path_and_var[node, neighbor][commodity_index].append((shortest_path, new_var))
...
def is_arc_in_path(path, arc):
    node, neighbor = arc

    for node_index in range(len(path) - 1):
        if node == path[node_index] and neighbor == path[node_index + 1]:
            return True
